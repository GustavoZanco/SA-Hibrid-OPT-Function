...
c
c addedd varconvex(:) above and in readparm and sgsim
c
...
c
c Added varconxfl*512 in character and read(lin,'(a512)',err=98) varconxfl
c allocated memory for varconvex and 
c-----------------------------------------------------------------------
      use       geostat
      include  'sgsim.inc'
      real      var(50)
     ...
     +          dbgfl*512,varconxfl*512,lvmfl*512,str*512
...
c
      ...
      read(lin,'(a512)',err=98) varconxfl
      call chknam(varconxfl,512)
      write(*,*) ' convex hull file = ',varconxfl(1:40)
...
c
     ...
     +                  ' due to insufficient memory.'
      ...
      allocate(varconvex(MXYZ),stat = test)
            if(test.ne.0)then
...
c
c Read the convex hull definition data:
c
      inquire(file=varconxfl,exist=testfl)
      if(.not.testfl) stop 'ERROR datafl does not exist!'
      open(lin,file=varconxfl,status='OLD')
      rewind(lin)
      read(lin,*)
      read(lin,*) nvari
      do i=1,nvari
      ...
      do i=1,nxyz
            read(lin,*)varconvex(i)
      end do
      
c
c Open the output file:
c
      open(lout,file=outfl,status='replace',form='unformatted',
     +                     access='direct', recl=4)
C       write(lout,210)
C  210  format('SGSIM Realizations')
C       write(lout,211) 1,nx,ny,nz,xmn,ymn,zmn,xsiz,ysiz,zsiz,nsim
C  211  format(i2,3(1x,i4),3(1x,g14.8),3(1x,g12.6),i4) 
C       write(lout,212)
C  212  format('value')
      return
c
...
c
c
c INPUT VARIABLES: <adicionar a variavel convex aqui para depois programar ela>
c
...
c
                  index = order(in)
                  if(varconvex(index).eq.0) go to 5
...
c
     ...
     +                                  zmax,ltail,ltpar,utail,utpar)
                  ...
                  write(lout, rec=ind+(isim-1)*nxyz) simval
            end do
...
c-----------------------------------------------------------------------
 ...
 28   format('1     0.5    1.0              ',
     +       '-nz,zmn,zsiz')
      write(lun,29)
 29   format('convex_hull.dat               ',
     +       '-file with convex hull')
      write(lun,30)
 30   format('69069                         ',
     +       '-random number seed')
      write(lun,31)
